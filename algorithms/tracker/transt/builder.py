...
def build_transt_tracker(network_config, evaluation_config, weight_path, device):
    ...
    data_preprocessor = TransTEvaluationDataProcessor(
        network_config['data']['area_factor']['template'], network_config['data']['area_factor']['search'],
        network_config['data']['template_size'], network_config['data']['search_size'], device, preprocessing_on_device)
    ...
    return TransTTracker(model, device, data_preprocessor, post_processor, bounding_box_post_processor)
