...
def construct_UAV123(constructor: SingleObjectTrackingDatasetConstructor, seed):
    ...
    for sequence_info in sequence_info_list:
        ...
        sequence_attr = np.loadtxt(attr_file_path, dtype=np.bool, delimiter=',')

        bounding_boxes = np.loadtxt(sequence_annotation_path, dtype=np.float, delimiter=',')
        ...
        with constructor.new_sequence(category_id) as sequence_constructor:
            ...
            '''
            sequence_constructor.set_attribute('attr', sequence_attr.tolist())
            for image in images:
                with sequence_constructor.new_frame() as frame_constructor:
                    frame_constructor.set_path(os.path.join(sequence_path, image))

            for index, bounding_box in enumerate(bounding_boxes):
                index_of_image = index + sequence_info['startFrame'] - 1
