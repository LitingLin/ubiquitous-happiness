...
class VisDrone2019_DET_Seed(BaseSeed):
    def __init__(self, root_path: str=None, data_split=DataSplit.Training):
        if root_path is None:
            if data_split == DataSplit.Training:
                root_path = self._getPathFromConfig('VisDrone_2019_DET_Training_PATH')
            elif data_split == DataSplit.Validation:
                root_path = self._getPathFromConfig('VisDrone_2019_DET_Val_PATH')
            elif data_split == DataSplit.Testing:
                root_path = self._getPathFromConfig('VisDrone_2019_DET_Test_PATH')
            else:
    ...
    def construct(self, constructor):
        from ..Sources.VisDrone_2019_DET import construct_VisDrone2019_DET
