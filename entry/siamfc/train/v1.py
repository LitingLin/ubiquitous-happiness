...
default_config_path = os.path.join(root_path, 'config', 'siamfc', 'v1')

import argparse
from pathlib import Path
import Utils.detr_misc as utils
from training.tracking.training_loop import training_loop
from training.siamfc.builder import build_training_actor_and_dataloader
...
def get_args_parser():
    parser = argparse.ArgumentParser('Set SiamFC tracker parameters', add_help=False)
    ...
    parser.add_argument('--train_config', type=str, default=os.path.join(default_config_path, 'training.yaml'), help='Path to the train config')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('SiamFC tracker training script', parents=[get_args_parser()])
