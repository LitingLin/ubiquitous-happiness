...
def curate_image_like_siamfc(image, center, scale, out_size, avg_color=None):
...
def curate_image_like_siamfc_with_bbox(image, center, scale, out_size, avg_color=None):
    if avg_color is None:
        avg_color = np.round(cv2.mean(image)[0:3]).astype(np.uint8)
    out_center = [out_size / 2, out_size / 2]
    interp_method = InterpolationMethod.INTER_LINEAR
...
def siamfc_z_curation_with_bbox(image, bounding_box, context, exemplar_size, avg_color=None):
    return curate_image_like_siamfc_with_bbox(image, *get_siamfc_curation_center_and_scale(bounding_box, context,
                                                                                          exemplar_size), exemplar_size, avg_color)


def siamfc_x_curation_with_bbox(image, bounding_box, context, exemplar_size, instance_size, avg_color=None):
    return curate_image_like_siamfc_with_bbox(image, *get_siamfc_curation_center_and_scale(bounding_box, context,
                                                                                          exemplar_size), instance_size, avg_color)
