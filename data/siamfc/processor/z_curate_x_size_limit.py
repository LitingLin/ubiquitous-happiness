...
class SiamFC_Z_Curate_BBOX_XYWH_X_SizeLimit_BBOX_CXCYWHNormalized_Processor:
    def __init__(self, exemplar_sz, instance_size_limit, context=0.5, do_image_normalize=True, return_torch_tensor=True):
        self.exemplar_sz = exemplar_sz
        self.instance_size_limit = instance_size_limit
        self.context = context
        self.x_resizer = ImageSizeLimiter(instance_size_limit)
    ...
    def get_x_bbox(self, bbox: numpy.ndarray, image_size):
        bbox = bbox_xywh2cxcywh_normalize_(bbox, image_size)
        if self.return_torch_tensor:
    ...
    def reverse_x_bbox(self, bbox, origin_image_size):
        ...
        bbox = bbox_denormalize_cxcywh2xywh(bbox, origin_image_size)
        return bbox
