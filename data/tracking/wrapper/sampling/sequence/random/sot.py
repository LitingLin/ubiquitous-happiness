...
class SingleObjectTrackingDatasetSequenceSampler:
    def __init__(self, number_of_objects = 1, frame_range_size=None, sampling_allow_duplication=True, sampling_allow_insufficiency=True, sort_result=False, rng_engine=np.random):
    ...
    def __call__(self, sequence: SingleObjectTrackingDatasetSequence_MemoryMapped):
        ...
        return _arg_sample(len(sequence), validity_flags_vector, self.number_of_objects, self.frame_range_size, self.sampling_allow_duplication, self.sampling_allow_insufficiency, self.sort_result, self.rng_engine)
