from data.tracking.methods.TransT.training.pipeline import TransT_training_data_preprocessing_pipeline, build_TransT_image_augmentation_transformer, TransT_training_image_preprocessing
import numpy as np
...
class TransTProcessor:
    ...
    def __call__(self, z_image, z_bbox, x_image, x_bbox, is_positive):
        ...
        z_image, z_bbox = TransT_training_data_preprocessing_pipeline(z_image, z_bbox, self.template_area_factor,
                                                                      self.template_size,
                                                                      self.template_scale_jitter_factor,
                                                                      self.template_translation_jitter_factor,
                                                                      self.transform)
        x_image, x_bbox = TransT_training_data_preprocessing_pipeline(x_image, x_bbox, self.search_area_factor,
                                                                      self.search_size,
        ...
        return z_image, x_image, target_feat_map_indices, target_class_label_vector, target_bounding_box_label_matrix, is_positive
