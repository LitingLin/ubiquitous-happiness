...
class TransTStage2DataProcessor:
    ...
    def __call__(self, samples, miscellanies_host, miscellanies_device):
        ...
        z_batch = torch.empty([len(z_indices_of_collated_tensors), 3, *self.template_size], device=z_indices_in_collated_tensors.device)
        x_batch = torch.empty([len(x_indices_of_collated_tensors), 3, *self.search_size], device=x_indices_in_collated_tensors.device)
