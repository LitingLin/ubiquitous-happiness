...
def build_siamfc_sampling_dataset_and_dataloader(args, train_config: dict, train_dataset_config_path: str, val_dataset_config_path: str, train_post_processor, val_post_processor, seed: int, collate_fn):
    if 'version' not in train_config or train_config['version'] < 2:
        ...
        train_dataset = build_siamfc_sampling_dataset(train_data_config, train_dataset_config_path, train_post_processor, rng_engine.integers(0, 1000000))
        train_data_loader = IterableDatasetOrchestrator(train_dataset, train_config['train']['batch_size'], rank_id, world_size, np.random.SeedSequence(rng_engine.integers(0, 1000000)))
        train_data_loader = torch.utils.data.dataloader.DataLoader(train_data_loader, train_config['train']['batch_size'], num_workers=args.num_workers, collate_fn=collate_fn)

        val_data_config = None
        if 'data' in train_config['val']:
            val_data_config = train_config['val']['data']

        val_dataset = build_siamfc_sampling_dataset(val_data_config, val_dataset_config_path, val_post_processor, rng_engine.integers(0, 1000000))
        ...
        val_data_loader = torch.utils.data.dataloader.DataLoader(val_data_loader, train_config['val']['batch_size'],
