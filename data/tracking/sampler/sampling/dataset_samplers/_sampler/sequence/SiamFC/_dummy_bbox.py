import numpy as np
from data.math.random.truncated_normal import truncated_normal_dist_generate_random_number
...
def generate_dummy_bbox_xyxy(image_size, rng_engine=np.random, reference_bbox: np.ndarray=None):
    ...
    dummy_bbox[0] *= image_size[0] - 1
    dummy_bbox[1] *= image_size[1] - 1
    dummy_bbox[2] *= image_size[0] - 1
    dummy_bbox[3] *= image_size[1] - 1

    return dummy_bbox


if __name__ == '__main__':
    print(generate_dummy_bbox_xyxy((20, 40)))
