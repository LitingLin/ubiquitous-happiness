...
class SingleObjectTrackingDatasetSequenceSampler:
    def __init__(self, data_getter, number_of_frames=1, frame_range_size=None, sampling_allow_object_absence=False, sampling_allow_duplication=True,
    ...
    def sample_indices(self, sequence: SingleObjectTrackingDatasetSequence_MemoryMapped):
        if not self.sampling_allow_object_absence and sequence.has_bounding_box_validity_flag():
            validity_flags_vector = sequence.get_all_bounding_box_validity_flag()
        else:
            validity_flags_vector = None

        return _arg_sample(
            len(sequence), validity_flags_vector, self.number_of_frames,
    ...
    def __call__(self, sequence: SingleObjectTrackingDatasetSequence_MemoryMapped):
        if not self.sampling_allow_invalid_bounding_box and sequence.has_bounding_box_validity_flag():
            validity_flags_vector = sequence.get_all_bounding_box_validity_flag()
        else:
            validity_flags_vector = None
