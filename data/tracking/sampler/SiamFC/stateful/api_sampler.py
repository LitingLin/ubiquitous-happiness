...
class SOTTrackingSiameseIterableDatasetApiGatewaySampler:
    def __init__(self, server_address, datasets, negative_sample_ratio, enforce_fine_positive_sample,
                 sampling_method: SiamesePairSamplingMethod, adaptive_frame_range,
                 default_frame_range = 100, datasets_sampling_parameters=None, datasets_sampling_weight=None, data_processor=None):
        self.sampling_orchestrator = ApiGatewayRandomSampler(server_address)
    ...
    def start(self):
        self.sampling_orchestrator.start()

    def stop(self):
        self.sampling_orchestrator.stop()
    ...
    def _move_next(self):
        index_of_dataset, index_of_sequence = self.sampling_orchestrator.get_next()
            ...
            if isinstance(dataset, DetectionDataset_MemoryMapped):
                ...
                if self.enforce_fine_positive_sample:
                    if isinstance(dataset, SingleObjectTrackingDataset_MemoryMapped):
                        sampled_data = do_positive_sampling_in_single_object_tracking_dataset_sequence(sequence, frame_range, self.sampling_method, rng_engine)
                    else:
                        sampled_data = do_positive_sampling_in_multiple_object_tracking_dataset_sequence(sequence, frame_range, self.sampling_method, rng_engine)
                    if len(sampled_data) != 2:
                        return None
                    data = (sampled_data[0][0], sampled_data[0][1], sampled_data[1][0], sampled_data[1][1], True)
                else:
                    if isinstance(dataset, SingleObjectTrackingDataset_MemoryMapped):
                        sampled_data, is_positive = do_sampling_in_single_object_tracking_dataset_sequence(sequence, frame_range, self.sampling_method, rng_engine)
                    else:
                        sampled_data, is_positive = do_sampling_in_multiple_object_tracking_dataset_sequence(sequence, frame_range, self.sampling_method, rng_engine)
                    if is_positive == 0:
