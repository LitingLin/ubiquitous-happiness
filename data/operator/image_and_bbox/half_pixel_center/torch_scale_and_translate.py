...
def torch_scale_and_translate_nchw(img: torch.Tensor, output_size, scale, input_center=(0, 0), output_center=(0, 0), background_color=(0, 0, 0), mode='bicubic'):
    n, c, h, w = img.shape
    dtype = img.dtype
    ...
    output_bbox = bbox_scale_and_translate((0, 0, w, h), scale, input_center, output_center)
    ...
    transformed_img = torch.nn.functional.interpolate(img[:, :, input_bbox[1]: input_bbox[3], input_bbox[0]: input_bbox[2]].float(), (in_range_bbox[3] - in_range_bbox[1], in_range_bbox[2] - in_range_bbox[0]), mode=mode, align_corners=False)
    output_img[:, :, in_range_bbox[1]: in_range_bbox[3], in_range_bbox[0]: in_range_bbox[2]] = transformed_img
    return output_img, output_bbox
