...
class GFocalV2Head(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, shape,
        ...
        self.shared_mlp = Mlp(input_dim, hidden_dim)
        self.classification_head = nn.Linear(hidden_dim, 1)
        self.regression_head = nn.Linear(hidden_dim, 4 * (gfocal_reg_max + 1))

        trunc_normal_(self.classification_head.weight, std=.02)
        trunc_normal_(self.regression_head.weight, std=.02)
    ...
    def forward(self, x):
        ...
        H = self.shape[1]
        W = self.shape[0]
        shared_feat = self.shared_mlp(x, H, W)
