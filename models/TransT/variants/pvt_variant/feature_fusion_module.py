...
class FeatureFusionEncoderLayer(nn.Module):
    def __init__(self, input_x_dim, input_y_dim, hidden_dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., sr_ratio=1, act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super(FeatureFusionEncoderLayer, self).__init__()
        self.x_input_proj = DWConv(input_x_dim, hidden_dim)
        self.y_input_proj = DWConv(input_y_dim, hidden_dim)
        ...
        self.x_mlp = Mlp(in_features=hidden_dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)

        self.y_mlp_norm = norm_layer(hidden_dim)
        self.y_mlp = Mlp(in_features=hidden_dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
