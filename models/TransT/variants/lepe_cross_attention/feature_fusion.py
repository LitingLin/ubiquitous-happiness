...
class Encoder(nn.Module):
    def __init__(self, dim, num_heads, z_size, x_size, # (H, W)
                 drop_path=0., position_encoder_spatial_locality=3, position_encoder_dim_locality=4):
        ...
        self.cross_attn = CrossAttention(dim, num_heads, z_size, x_size, drop_path=drop_path,
                                         position_encoder_spatial_locality=position_encoder_spatial_locality,
                                         position_encoder_dim_locality=position_encoder_dim_locality)
...
class FeatureFusionNetwork(nn.Module):
    def __init__(self, hidden_dim, n_heads, template_size, search_size, n_feature_fusion_layers, drop_path_rate,
        ...
        self.encoder = ModuleList(
            [Encoder(hidden_dim, n_heads, template_size, search_size, drop_path=dpr[i],
                     position_encoder_spatial_locality=position_encoder_spatial_locality,
                     position_encoder_dim_locality=position_encoder_dim_locality) for i in range(n_feature_fusion_layers)]
        )
...
def build_feature_fusion_network(network_config: dict):
    ...
    return FeatureFusionNetwork(transformer_config['hidden_dim'], transformer_config['nheads'],
                                ...
                                transformer_config['position_encoder_spatial_locality'],
                                transformer_config['position_encoder_dim_locality'])
