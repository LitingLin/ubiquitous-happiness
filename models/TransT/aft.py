...
class RelativePositionCrossAttention(nn.Module):
    def __init__(self, dim, num_heads, z_size, x_size, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.): # h, w
        ...
        for h_z in range(h_z_begin, h_z_end):
            for h_x in range(h_x_begin, h_x_end):
                for w_z in range(w_z_begin, w_z_end):
                    ...
                    relative_position_index.append(relative_position_index_[z_position][x_position_begin: x_position_end])
        relative_position_index = torch.cat(relative_position_index)
        self.register_buffer("relative_position_index", relative_position_index)
        self.relative_position_bias_table = nn.Parameter(torch.zeros(((2 * position_bias_table_size[0] - 1) * (2 * position_bias_table_size[1] - 1), num_heads)))
...
if __name__ == '__main__':
    ...
    d=RelativePositionCrossAttention(128, 8, [8, 8], [16, 16])

    e = torch.zeros([3, 8*8, 128])
    f = torch.zeros([3, 16*16, 128])
    d(e, f)
