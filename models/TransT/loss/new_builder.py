...
def build_criterion(network_config: dict, train_config: dict, iterations_per_epoch: int):
    ...
    if 'data_filter' in loss_parameters:
        global_data_filter_build_function_module = importlib.import_module(head_loss_fn_module_prefix + '.data_filter.' + loss_parameters['data_filter'])
    ...
    for loss_cluster_module_name, loss_cluster_module_parameters in loss_parameters.items():
        ...
        for loss_name, loss_parameters in loss_cluster_module_parameters.items():
            if loss_name == 'reduction' or loss_name == 'data_filter':
                continue
        ...
        if 'data_filter' in loss_cluster_module_parameters:
            sample_filter_parameters = loss_cluster_module_parameters['data_filter']
            sample_filter_function_type = sample_filter_parameters['type']
            sample_filter_function_module = loss_cluster_module_prefix + '.data_filter.' + sample_filter_function_type
    ...
    criterion = SingleScaleCriterion(global_data_filter, loss_modules)
